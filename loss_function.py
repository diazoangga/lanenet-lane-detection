...
class CalculateIOU(tf.keras.metrics.Metric):
    def __init__(self, num_classes):
        super(CalculateIOU, self).__init__()
        self.f_iou = MeanIoU(num_classes)
    def update_state(self, y_true, y_pred, sample_weight=None):
        y_pred = tf.argmax(y_pred, axis=-1)
        y_true = tf.squeeze(y_true, axis=-1)
        self.f_iou.update_state(y_true, y_pred, sample_weight)
    def reset_state(self):
        self.f_iou.reset_state()
    def result(self):
        return self.f_iou.result()
...
def instance_loss(instance_label, net_out):
    ...
    _, var_loss, dist_loss, reg_loss = discriminative_loss(inst_seg, instance_label, 3, (256, 512))
